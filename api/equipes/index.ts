...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'GET') {
    ...
    const equipes = baseId
      ? await sql`
          select id, base_id, tipo, identificador, ativo
          from equipes
  ...
  if (req.method === 'POST') {
    ...
    const { baseId, tipo, identificador } = req.body || {};
    ...
    try {
      const [equipe] = await sql`
        insert into equipes (base_id, tipo, identificador, ativo)
        values (${baseId}, ${tipo}, ${identificador}, true)
