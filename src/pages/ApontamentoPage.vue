...
// Técnico/Coordenador não escolhem base: veem todas as equipes do seu
// supervisor/coordenador, mesmo que estejam espalhadas por bases diferentes.
...
const equipesComBuscaTipo = computed(() =>
  equipesBase.value.filter((e) => {
    ...
    const bateTipo = tipoFiltro.value.length === 0 || tipoFiltro.value.includes(e.tipo);
...
const equipesFiltradas = computed(() =>
  equipesComBuscaTipo.value.filter((e) => statusFiltro.value.length === 0 || statusFiltro.value.includes(e.status)),
);

const temFiltroAtivo = computed(() => !!busca.value || tipoFiltro.value.length > 0 || statusFiltro.value.length > 0);
